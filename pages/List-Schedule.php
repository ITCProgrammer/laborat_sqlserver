...
<body>
  ...
  <div class="modal fade" id="ModalElement" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">

        <form id="ModalElementForm" action="your-submit-url.php" method="POST">
          <div class="modal-header">
            <h5 class="modal-title">Update Element</h5>
          </div>
          ...
          <div class="modal-body">
            ...
            <!-- No Resep -->
            <div class="mb-3">
              <label class="form-label fw-semibold">No. Resep</label>
              <div class="form-control-plaintext" id="ModalElementResep"></div>
            </div>

            <!-- No Order -->
            <div class="mb-3">
              <label class="form-label fw-semibold">No. Order</label>
              <div class="form-control-plaintext" id="ModalElementOrder"></div>
            </div>

            <!-- Element Code (center focus) -->
            <div class="mb-3 text-center">
              <label class="form-label fw-semibold">Element Code</label>
              <input
                type="text"
...
<script>
  $(document).ready(function() {
    ...
    $(document).on("click", ".openElementModal", function(e) {
      e.preventDefault();

      const resep = $(this).data("resep");
      const order = $(this).data("order");
      const element = $(this).data("element");

      // set value ke modal
      $("#ModalElementResep").html(resep);
      $("#ModalElementOrder").html(order);
      $("#ModalElementVal").val(element);
      ...
      $('#ModalElement').on('shown.bs.modal', function() {
        $("#ModalElementVal").trigger('focus').select();
      });
    });

    $("#ModalElementForm").on("submit", function(e) {
      e.preventDefault(); // hentikan form submit biasa

      // kumpulkan payload
      const payload = {
        no_resep: $("#ModalElementResep").text().trim(),
        element_code: $("#ModalElementVal").val().trim(),
      };

      $.ajax({
        url: "pages/ajax/Insert_ResepElement.php",
        type: "POST",
        data: payload,
        dataType: "json",
        success: function(res) {
          if (res.status === "success") {
            ...
            Swal.fire({
              icon: 'success',
              title: (res.mode === 'insert') ? 'Element Ditambahkan' : 'Element Diupdate',
              text: res.message,
              timer: 1500,
              showConfirmButton: false
            }).then(() =>
