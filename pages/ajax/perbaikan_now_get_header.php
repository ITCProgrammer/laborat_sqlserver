...
// - Perbaikan NOW (form NowForm di Form-Matching)
...
try {
    // 1. Langganan (PROJECTCODE + LANGGANAN/BUYER)
    $sqlLangganan = "
        SELECT 
    ...
    if (!$stmtLangganan || !db2_execute($stmtLangganan, ['%' . $reqNo . '%'])) {
        throw new Exception('Gagal mengambil data langganan (Matching Ulang NOW / Perbaikan NOW).');
    }
    ...
    if (!$dt_langganan) {
        echo json_encode([
            'success' => false,
            'message' => 'Data Matching Ulang NOW / Perbaikan NOW tidak ditemukan untuk Req No tersebut.'
        ]);
        exit;
    }
