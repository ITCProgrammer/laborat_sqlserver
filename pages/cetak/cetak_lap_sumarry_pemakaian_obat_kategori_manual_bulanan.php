           ...
           <td colspan="2" 
            ...
            <td colspan="8" style="width: 60%; text-align: center; vertical-align: middle; height: 80px;">
                ...
                $db_stocktransaction = db2_exec($conn1, "SELECT
																...
																LISTAGG(
																	TRIM(p.SUBCODE01) || '-' || TRIM(p.SUBCODE02) || '-' || TRIM(p.SUBCODE03),
																	','
																) WITHIN GROUP (
																	ORDER BY p.SUBCODE01, p.SUBCODE02, p.SUBCODE03
																) AS KODE_OBAT
															FROM PRODUCT p
															LEFT JOIN ADSTORAGE a3 
																   ON a3.UNIQUEID = p.ABSUNIQUEID 
																  AND a3.FIELDNAME = 'ShowChemical'
															LEFT JOIN USERGENERICGROUP u 
																   ON u.CODE = p.SUBCODE01 
																  AND u.USERGENERICGROUPTYPECODE = 'S09'
															WHERE 
																p.ITEMTYPECODE = 'DYC'
...
<br><br>
    <table border="1">
       ...
       while ($row = db2_fetch_assoc($db_stocktransaction)) {                                    
								$kode_obat_list = "'" . str_replace(",", "','", $row['KODE_OBAT']) . "'";
...
--                                            AND s.TRANSACTIONDATE BETWEEN '$awal' AND '$akhir'
                            ...
                            $stock_minimum = db2_exec($conn1, " SELECT 
                                    ...
                                    $buka_po = db2_exec($conn1, "SELECT 
											...
											AND TRIM(DECOSUBCODE01) || '-' || TRIM(DECOSUBCODE02) || '-' || TRIM(DECOSUBCODE03) IN ($kode_obat_list)
                                            GROUP BY 
                                            LOGICALWAREHOUSECODE,
                                            COUNTERCODE,
                                            DECOSUBCODE01,
            ...
            $Balance_stock = sqlsrv_query($con, $sql, $params);
